...
/**
 * 第 0 層:無鎖對照組。**本實作會超賣,而且那是刻意的。**
 *
 * <p>流程為「讀取庫存 → 於領域層計算 → 寫回絕對值」。兩個併發交易讀到相同的 available、
 * 各自計算後寫回同一個值,其中一次更新就此遺失(lost update),而兩筆訂單都成立。
 *
 * <p><b>本類別標註 {@code @Transactional},但那不是為了正確性。</b> 四層策略的交易邊界必須一致,
 * 否則比較出來的是「有無交易」而非「有無鎖」。它同時使本層成為一個普遍誤解的反例:
 * 交易保證原子性,不保證併發下的互斥 —— PostgreSQL 預設的 READ COMMITTED
 * 完全允許上述情境發生。
 *
