...
-- 讓已購數的期限**繼承庫存的絕對到期時間**,再加上一段緩衝。
--
-- 繼承而非讓呼叫端各自指定:分別指定等於把一條安全條件交給每一個呼叫端記得。
--
-- **緩衝不可省,而且方向不能反。** 兩者的失敗方向不對等:
--   庫存先過期   → 場次視為「未開賣」,購票停止 —— fail-closed,安全
--   已購數先過期 → 限購額度歸零,可以重新買一輪 —— 直接超買
--
-- 「設成相同的到期時間」並不安全:Redis 的過期是**惰性 + 抽樣**的,
-- key 不在到期的瞬間消失,而是被存取時或被背景取樣掃到時才移除。
-- 兩個到期時間相同的 key,完全可能出現「一個已消失、另一個還在」的窗口 ——
-- 而其中一半的可能性正是不安全的那一半。
--
-- 庫存沒有過期時間(EXPIRETIME 回 -1)時,已購數也不設 ——
-- 保持一致,不走向不安全的方向。那個情況由對帳的警告負責讓人看見。
local deadline = redis.call('EXPIRETIME', KEYS[1])
if deadline > 0 then
  redis.call('EXPIREAT', KEYS[2], deadline + tonumber(ARGV[3]))
end
