...
- 🚫 **Never inject `PurchaseTicketUseCase` directly — including via `@Qualifier` or `@Primary`.** Access is only through `PurchaseFacade`'s `Map<String, PurchaseTicketUseCase>`. A plain single injection already fails at startup with `NoUniqueBeanDefinitionException`, so that is not what this rule is for — **it exists to stop someone making it "work" with a qualifier**, which silently binds that class to one strategy and destroys the "one API, four implementations" premise with no error message. 〔**測試**〕
- 🚫 **Never rely on `synchronized` to make a `@Transactional` method concurrency-safe** — the lock is narrower than the transaction. `@Transactional` is an AOP proxy: the commit happens *after* the method returns, so the next thread enters the synchronized block before the previous transaction is visible. Measured on this project: adding `synchronized` to the no-lock strategy still oversold 47 tickets. 〔**自律**〕
...
- 🚫 **Never call a `@Transactional` method from inside the same class** — self-invocation bypasses the AOP proxy and the transaction silently does not apply. This is the single most common Spring mistake. It was self-discipline only until the optimistic-lock layer, where `transactionalMethodsAreNeverSelfInvoked` (a custom ArchUnit condition that walks the call graph) turned it into a check. 〔**測試**〕
- 🚫 **Never put `@Transactional` on the class that holds a retry loop** — the connection stays checked out for the entire loop. Under 1000 concurrent requests with a pool of 50, a retry storm drains the pool and what you measure becomes "waiting for a connection" rather than "retrying"; the pessimistic layer already proved those two are indistinguishable on a latency chart. The loop and the single attempt must be separate beans. 〔**測試**〕
- 🚫 **Never restore cached stock while the order stream still has a backlog** — a backlog entry is a deduction that is still on its way to becoming an order. Restoring it and then having the message land anyway is a double count, i.e. overselling. "Backlog" means both `XPENDING` **and** undelivered entries: `XPENDING` only counts messages already handed to a consumer, so a lagging consumer produces moments where pending is 0 while hundreds of entries still sit in the stream. Reconciliation is deliberately biased: **missing a restore costs a few unsold tickets, a wrong restore costs correctness.** 〔**測試**〕
- 🚫 **Never restore cached stock with `SET`** — `INCRBY`/`DECRBY` keep a key's TTL, `SET` clears it. Rewriting a restore as "read, compute, write back" looks entirely reasonable and silently drops the expiry, after which `purchased:{event}:{user}` accumulates forever with no test failing and no error logged. Restores are always incremental. 〔**測試**〕
- 🚫 **Never treat a duplicate-key violation as a persistence failure in the stream consumer** — it means the message was already persisted, so the ticket is sold. Compensating for it hands that stock back while the order stays in the database. The crash window between compensating and acking cannot be closed, so the idempotency key is what makes redelivery harmless; that is the whole reason `uq_purchase_order_idempotency_key` exists. 〔**測試**〕
...
- 🚫 **Never report a throughput number without its spread** — a single measurement carries no reliability information, and this project has measured 52% spread on the same configuration. `run-load-test.sh` always prints each run, the median and the range; quote all three. Two numbers whose gap is smaller than the noisier one's range do not support a conclusion — say "the data cannot tell them apart" rather than picking the favourable run. Keep the range formula fixed at `(max-min)/median`: switching the denominator to `min` turns 52% into 72% and would let any fix look effective. 〔**自律**〕
- 🚫 **Never compare groups that were measured one-after-another** — "measure group A three times, then group B three times" concentrates each group's samples in one stretch of the timeline, so any environmental drift becomes a **bias that favours specific groups** rather than noise shared by all, and bias cannot be removed by measuring more times. Comparison data comes from `k6/run-comparison-matrix.sh` — one round covers every group, rounds rotate the order, and the batch carries its own drift verdict. Measured on this project: the virtual-thread groups' spread was 20.6%–53.1% when they all ran after the platform groups, and 4.0%–7.7% once interleaved — **the "virtual threads are more variable" reading was an artefact of position, not a property of the thread model.** 〔**自律**〕
- 🚫 **Never treat a single batch's conclusion as established** — the spread rule ("gap must exceed the noisier range") guards against noise *within* a batch; it says nothing about whether the same batch would come out the same way tomorrow. Measured on this project: one change concluded a difference was real (gap 9.02% against a 4.92% range); the very next change re-measured the same two configurations and could not distinguish them at all (gap 1.42% against a 7.75% range). **Both batches passed every acceptance check.** Before a number becomes a claim, either reproduce it or say plainly that it rests on one batch. And when a quantity drifts with the machine's speed, prefer a ratio against something that drifts with it — on this project the absolute figure moved 11% between batches while the ratio moved 2%. 〔**自律**〕
- 🚫 **Never fold two things into one number when they need different fixes** — a combined figure looks authoritative and quietly removes the distinction that tells you what to do. This project has hit it three times in four changes: combined CPU could not separate app work from kernel scheduling (the combined figure supported no conclusion; splitting user/system did); a combined throttle count blamed the app when postgres was the one hitting its quota, and an entire change was designed on that wrong premise; a combined connection-pool time would have said "add connections" when the group that waited longest was the *fastest* — throughput tracked how long a connection was **held**, not how long it was **waited for**. Aggregate for a verdict if you like, but **diagnostics must stay split**, because the split is the part that names the fix. 〔**自律**〕
- 🚫 **Never attribute a performance difference to a mechanism without a measurement of that mechanism** — naming a cause ("the requests are queueing at the row lock", "the extra threads cost more") requires evidence beyond throughput, because **throughput is the outcome, not the mechanism**. Two configurations can have the same throughput at twice the CPU, or the same CPU at twice the throughput; one means work is being wasted and the other means work is waiting, and **on a throughput chart they are identical**. When CPU is the evidence, split user from system time — thread creation, switching and scheduling are kernel work, and the combined figure cannot separate "the app did more" from "the kernel worked harder". Measured on this project: raising the platform admission limit cost 17.2% more system time per request while the database's per-request cost was indistinguishable — the combined CPU figure supported no conclusion at all. Without such a measurement, write "a possible explanation" and mark it as unverified; **an explanation with no evidence looks exactly like a conclusion.** 〔**自律**〕
- 🚫 **Never report a performance number without its measurement conditions** — strategy, thread model (platform / virtual), **the request admission limit**, container CPU and memory limits **for every container in the path — the app is rarely the one that saturates**, `max_connections`, k6 VU count and duration, and **that the host carried no other workload**. Measured on this project: one unrelated MSSQL container started eight hours earlier, plus swap at 81%, dropped throughput from 7300 to 5800 req/s and degraded two consecutive batches — and no guard caught it, because every guard watches the system under test rather than the machine under it. The admission limit is listed separately from the thread model on purpose: the thread model says *what kind of thread*, not *how many requests are let in at once*, and it is the latter that decides how many requests contend for the connection pool and the locked row. Measured on this project: platform threads were implicitly capped at Tomcat's default of 200 while virtual threads had no such cap — a difference that sat in every comparison for five changes without appearing in any conditions table. A number without conditions cannot be compared to another, which makes it worthless in a project whose entire output is comparison. 〔**自律**〕
